<%@page import="music.Diff_day"%>
<%@page import="music.Cal_Age"%>
...
DAO dao = new DAO();
JSONObject json = dao.music_foryou_date_num();
...
<script type="text/javascript">
	...
	var id = "<%=id%>";
	var score_date_numplay;
	
	function score(x){
		score_date_numplay;
	}
	$(function() {
		console.log(score(json.music[0].date));
	})
	function music_play(playlist_id) {
		...
		$('#pl-musiclist-block-' + playlist_id).css('color', '#fff')
		$.ajax({
			url : "num.jsp",
			data : {
				id : music_id
	...
	$(
			function() {
				...
				for (var i = 0; i < memberData.music.length; i++) {
					$('#m_content')
							.append('<div id="m_'+i+'"><div style="cursor:pointer;" id="#music'
									+ i
									+ '" onclick="music_plus('
									+ i
									+ ');"><img alt="'+memberData.music[i].title+'"src="'+memberData.music[i].img+'" width="200px" style="font-weight:bolder;border-radius: 10px;"></div><br> <a style="font-size: 20px;font-weight:bolder;">'
		...
		<aside class="c-nav">
			<nav id="nav">
				...
				<ul>
					...
					<li
						style="margin-bottom: 15px; padding: 5px; font-weight: bolder; background-color: #ffe1e2;"><a
			...
			<div class="m_main" style="padding: 20px; margin-top: 50px">
				...
				<div>
					<div class="music_box" style="padding: 10px;">
						<h3 style="margin: 10px;">나이대 별 추천</h3>
					</div>
					<div>
						<div id="m_content"
