...
JSONObject jo1 = dao.music_top();
JSONObject jo2 = dao.music_foryou_date_num();
%>
<!DOCTYPE html>
<html>
<head>
		...
		$('#pl-musiclist')
				.prepend(
						'<li id="pl-musiclist-block-'+ playlist_id +'" class="'+memberData.music[x].id+'" style="border-bottom:1px solid darkgray; padding:5px; border-radius:5px; background:#ffe1e2;"><div style="display: grid; grid-template-columns: [left-edge] 50px[artwork-edge] calc(100% - 112px)[title-edge] 40px[right-edge]; grid-column-gap: 16px;"><a href="#"><img style="border-radius: 5px;" width="100%" src="'
								...
								+ '"></a><div><a href="#" style="font-size: 20px; style="font-size: 20px; font-weight:bolder;">'
...
var list_array = [];
	$(
			function() {
				...
				/* $.ajax({
					url:"load_playlist.jsp",
					data:{
						id : id
					},
					success: function(data) {
						list_array = data.split(',');
						for (var i = 0; i < list_array.length; i++) {
							$.ajax({
								...
								success : function(json) {
									 json3 =json;
									 jsondata3 = JSON.stringify(json3);
									 memberData3 = JSON.parse(jsondata3);
									 $('#pl-musiclist')
										.prepend(
												'<li id="pl-musiclist-block-'+ playlist_id +'" class="'+memberData3.music[0].id+'" style="border-bottom:1px solid darkgray; padding:5px; border-radius:5px; background:#ffe1e2;"><div style="display: grid; grid-template-columns: [left-edge] 50px[artwork-edge] calc(100% - 112px)[title-edge] 40px[right-edge]; grid-column-gap: 16px;"><a href="#"><img style="border-radius: 5px;" width="100%" src="'
														+ memberData3.music[0].img
														+ '"></a><div><a href="#" style="font-size: 20px; style="font-size: 20px; font-weight:bolder;">'
														+ memberData3.music[0].title
														+ '</a><br><a href="#">'
														+ memberData3.music[0].artist
														+ '</a></div><div><img src="img/play_icon.png" onclick="music_play('
														+ playlist_id
														+ ')" width="23px" style="cursor: pointer;"><img src="img/x_icon.png" onclick="music_delete('
														+ playlist_id
				...
				memberData1 = JSON.parse(jsondata1);
				
				for (var i = 0; i < 20; i++) {
					$('#m_content_2')
							.append(
									'<div style="display: grid; grid-template-columns: [left-edge] 50px[artwork-edge] 100px[title-edge] 70px[right-edge] 20px; grid-column-gap: 5px; border-bottom: 1px solid lightgray; padding:5px;height:62px;"><div><img alt="" src="'
									+memberData1.music[i].img+'" width="100%" style="border-radius: 5px;"></div><div style="text-align: center;"><div style="display:inline-block"><a href="#" style="text-align:left; font-weight:bolder; font-size:12px">'
									+memberData1.music[i].title+'</a></div></div><div style=""><a href="#" style="font-size: 10px">'
									+memberData1.music[i].artist+'</a></div><div style=""><img src="img/play_icon.png" onclick="music_plus('
										+ i
										+ ')" width="23px" style="cursor: pointer;"></div></div>');
				}
				
				json2=<%=jo2%>;
				jsondata2 = JSON.stringify(json2);
				memberData2 = JSON.parse(jsondata2);
				if(id!="null"){
				for (var i = 0; i < 5; i++) {
					$('#m_content_3')
							.append(
									'<div id="m_'+i+'"><div style="cursor:pointer;" id="#music'
											...
											+ '" onclick="music_plus('
											+ i
											+ ');"><img alt="'+memberData2.music[i].title+'"src="'+memberData2.music[i].img+'" width="200px" style="border-radius: 10px;"></div><br> <a style="font-size: 20px; font-weight:bolder;">'
											+ memberData2.music[i].title
											+ '</a><br> <a>'
											+ memberData2.music[i].artist
											+ '</a></div>');
				}
...
<body>
	<div class="apple-music">
		<aside class="c-nav">
			<nav id="nav">
				...
				<ul>
					...
					<li
						style="margin-bottom: 15px; padding: 5px; font-weight: bolder;"><a
		...
		<main style="width: 2000px; margin-left: 220px;">
			...
			<div class="m_main" style="padding: 20px; margin-top: 50px; width:1200px;">
				<div class="music_box" style="border-bottom: 1px solid light-gray ;  padding: 10px;">
					<h2 style="margin: 10px;text-align: center;"><img alt="" src="img/full_black.png" width="25px">   AppleMango Music</h3>
				</div>
				<div>
				<div class="music_box" style="border-bottom: 1px solid light-gray ;  padding: 10px;">
					...
					<div id="m_content_1"
						style="display: grid; grid-template-columns: repeat(5, 200px); grid-auto-columns: 200px; grid-gap: 40px; flex-flow: row wrap;">

					</div>
					<div class="music_box" style="border-bottom: 1px solid light-gray ;  padding: 10px;">
					<h3 style="margin: 10px;">Hot Music</h3>
				</div>
					<div id="m_content_2"
						style="display: grid; grid-template-columns: repeat(4, 300px); grid-auto-columns: 200px; grid-gap: 2px; flex-flow: row wrap;">
						
					</div>
				<div class="music_box" style="border-bottom: 1px solid light-gray ;  padding: 10px;">
					<h3 style="margin: 10px;">For You</h3>
				</div>
					<div id="m_content_3"
						style="display: grid; grid-template-columns: repeat(5, 200px); grid-auto-columns: 200px; grid-gap: 40px; flex-flow: row wrap;">
