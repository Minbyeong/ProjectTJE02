				...
				.ajax({
					...
					success : function(data) {
						...
						$('#info_playlcd')
								.html(
										'<a><b>'
												+ memberData1.music[0].title
												+ '</b></a><br><a style="font-size: 12px">'
												+ memberData1.music[0].artist
												+ '</a><input type="range" name="" id="currentTime_played" value="0" oninput="currenttime_played_bar(this.value)" style="position:absolute; left:10px; top:30px;" >');
...
function music_plus1(x) {
		$('#pl-musiclist')
				.prepend(
						'<li id="pl-musiclist-block-'+ playlist_id +'" class="'+json_s.music[x].id+'" style="border-bottom:1px solid darkgray; padding:5px; border-radius:5px; background:#ffe1e2;"><div style="display: grid; grid-template-columns: [left-edge] 50px[artwork-edge] calc(100% - 112px)[title-edge] 40px[right-edge]; grid-column-gap: 16px;"><a href="#"><img style="border-radius: 5px;" width="100%" src="'
								+ json_s.music[x].img
								+ '"></a><div><a href="#" style="font-size: 20px">'
								+ json_s.music[x].title
								+ '</a><br><a href="#">'
								+ json_s.music[x].artist
								+ '</a></div><div><img src="img/play_icon.png" onclick="music_play('
								+ playlist_id
								+ ')" width="23px" style="cursor: pointer;"><img src="img/x_icon.png" onclick="music_delete('
								+ playlist_id
	...
	$(function() {
		...
		$('#search_bar').focusout(function() {
			...
			if(Object.keys(json_s.music).length ==41){
			...
			$.ajax({
				...
				success : function(data) {
					...
					for (var i = 0; i < 10; i++) {
						$('#search_box')
								.append(
										'<div style="display: grid; grid-template-columns: [left-edge] 50px[artwork-edge] calc(100% - 112px)[title-edge] 40px[right-edge]; grid-column-gap: 16px;">'+
										'<a href="#"><img style="border-radius: 5px;" width="100%" src="'+ json_s.music[i].img + '"></a><div><a href="#" style="font-size: 20px">'
												+ json_s.music[i].title
												+ '</a><br><a href="#">'
												+ json_s.music[i].artist
												+ '</a></div><div><img src="img/play_icon.png" onclick="music_plus1('
												+ i
												+ ')" width="23px" style="cursor: pointer;"></div></div>')
		...
		$('#playlist_icon').click(function() {
			if ($('#playlist_icon').css('fill') == 'rgb(128, 128, 128)') {
				$('#playlist_icon').css('fill', 'rgb(252,38,61)');
				$('#playlist').css('display', 'block');
			} else {
				$('#playlist_icon').css('fill', 'rgb(128,128,128)')
	...
	<div class="apple-music">
		<aside class="c-nav">
			<nav id="nav">
				...
				<ul>
					<input id="search_bar" type="text" width="100px"
						placeholder="Search" style="margin-bottom: 15px">
				...
				<div id="search_box"
					style="display: none; width: 250px; position: fixed; left: 20px; top: 100px; background: #fff; border: 2px solid black; border-radius: 10px;">
		...
		<main style="width: 2000px; margin-left: 220px;">
			...
			<div class="m_main" style="padding: 20px; margin: 50px 0px 0px 210px">
				<div class="music_box" style="padding: 10px;">
					<h3 style="margin: 10px;">New Music</h3>
				</div>
				<div>
					<div id="m_content" style="display: grid; grid-template-columns: repeat(4, 250px); grid-auto-columns: 200px; grid-gap: 20px; flex-flow: row wrap;">
						<div id="Bass" class="genre_set"
							Style="cursor:pointer; height: 200px; font-weight: bolder; font-size: 30px; line-height: 270px; border-radius: 20px; padding: 20px; background: linear-gradient(to left, #C6FFDD, #FBD786, #f7797d); color: white;">Bass</div>
						<div id="Trap" class="genre_set"
							Style="cursor:pointer; height: 200px; font-weight: bolder; font-size: 30px; line-height: 270px; border-radius: 20px; padding: 20px; background: linear-gradient(to right, #2c3e50, #bdc3c7); color: white;">Trap</div>
						<div id="House" class="genre_set"
							Style="cursor:pointer; height: 200px; font-weight: bolder; font-size: 30px; line-height: 270px; border-radius: 20px; padding: 20px; background: linear-gradient(to right, #f64f59, #c471ed, #12c2e9); color: white;">House</div>
						<div id="Chill" class="genre_set"
							Style="cursor:pointer; height: 200px; font-weight: bolder; font-size: 30px; line-height: 270px; border-radius: 20px; padding: 20px; background: linear-gradient(to right, #ec2F4B, #009FFF); color: white;">Chill</div>
						<div id="Electric" class="genre_set"
							Style="cursor:pointer; height: 200px; font-weight: bolder; font-size: 30px; line-height: 270px; border-radius: 20px; padding: 20px; background: linear-gradient(to right, #333333, #dd1818); color: white;">Electric</div>
						<div id="Melodic Dubstep" class="genre_set"
							Style="cursor:pointer; height: 200px; font-weight: bolder; font-size: 26px; line-height: 270px; border-radius: 20px; padding: 20px; background: linear-gradient(to right, #4568DC, #B06AB3); color: white;">Melodic Dubstep</div>
						<div id="Indie" class="genre_set"
							Style="cursor:pointer; height: 200px; font-weight: bolder; font-size: 30px; line-height: 270px; border-radius: 20px; padding: 20px; background: linear-gradient(to right, #0099F7, #F11712); color: white;">Indie</div>
						<div id="Future House" class="genre_set"
							Style="cursor:pointer; height: 200px; font-weight: bolder; font-size: 30px; line-height: 270px; border-radius: 20px; padding: 20px; background: linear-gradient(to right, #2F7336, #AA3A38); color: white;">Future
							House</div>
		...
		<div id="playlist"
			style="padding: 10px; border: 1px solid lightgray; position: fixed; right: 0; top:0; height: 100%; width: 300px; background: #f9f9f9; overflow: auto; display: none; opacity: 0.95;">
			<h3>PlayList</h3>
			<hr color="lightgray">
