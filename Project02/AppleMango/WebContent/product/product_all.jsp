...
var user_id = "<%= user_id %>"
	$(document).ready(function() { // 페이지가 시작하면 세션에 사용자의 아이디 값이 잡혀있는 지를 판단하고 로그인/로그아웃 상태로 만들어 놓는다
		if (user_id != "null") {
			$('#login').text('Logout')
		} else {
			$('#login').text('Login')
	...
	$('#cart').click(function() { // 장바구니 목록을 보기 위해선 사용자의 아이디가 필요하기 때문에 아이디의 값을 체크하고 세션에 값이 있으면 장바구니로 넘기고 
					if (user_id == "null") {  // 그렇지 않을경우 로그인 페이지로 화면을 넘겨준다.
...
<body id="page-top">
	...
	<div id="product1">
		...
		<div id="imac1">
			...
			<p>
				<a id="button" href="product_com.jsp?p_id=PC001">구입하기</a> <!-- 버튼을 눌러서 페이지를 넘길 때, 해당 상품의 정보를 불러오기 위해서 상품의 아이디 값을 같이 넘기고 있다 -->
			</p>
		</div>
		<div id="imac2">
