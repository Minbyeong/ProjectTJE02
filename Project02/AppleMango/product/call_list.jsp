...
<%
	...
	String user_id = (String) session.getAttribute("id"); <!--세션에 있는 사용자의 id를 받아서 user_id로 저장하고-->
	String p_id = request.getParameter("p_id");
	ProductDAO dao = new ProductDAO();
	JSONObject jo = dao.shopping(user_id); <!--저장된 사용자의 id값을 이용해서 장바구니 목록을 json으로 불러오고 있다.-->
%>
<!DOCTYPE html>
<html>
<head>
...
<%=jo%>
	...
	$(function() {
		for (var i = 0; i < Object.keys(json.shopping).length; i++) { //json으로 저장된 전체 장바구니 목록을 그 크기에 맞게 아래 방향으로 생성 
			idValue = i;
			$('#main-col')
					.prepend(
		...
		for (var i = 0; i < Object.keys(json.shopping).length; i++) { // 장바구니 목록에 있는 전체 상품의 가격을 total변수를 선언하고 전부 더하여 화면에 찍어주고 있다.
		...
		$('.delete-item').click(function() {
			...
			console.log(idValue)
			$.ajax({ //상품의 목록을 삭제하는 것은 현재의 페이지에서 처리해야 하기 때문에 ajax를 통해서 처리
				url : 'deletedb.jsp',
				data : {
		...
		$("#check_module").click(function() { //결제 api, 아임포트 사용
