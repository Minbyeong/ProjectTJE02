...
<%
	...
	ArrayList<ProductSubVO> list1 = dao.subcon(p_id); <!--옵션들에 대한 정보 전체를 리스트에 담아 놓는다.-->
	String[] sub = new String[list1.size()]; <!--가져온 값을 사용하기 위해서 배열을 만들어 값을 담아주었다.-->
...
<script type="text/javascript">
	...
	//총합계산
				...
				//db저장
				if (user_id == "null") { // 아이디의 값을 체크해서 없으면 로그인 페이지로 넘겨주고
					...
					$('#go-db')
							.click(
									function godb() { // 로그인이 되어있다면, 해당 정보들을 다음 페이지로 넘기면서 저장해 준다.
										con = con1 + '<br>' + con2 + '<br>'
												+ con3 + '<br>' + con4 + '<br>';
										prc = end_price
										console.log(con)
