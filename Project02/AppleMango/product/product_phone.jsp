...
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<%
	String user_id = (String) session.getAttribute("id"); <!--세션에서 넘겨 받은 아이디 값을 가져온다-->
	String p_id = request.getParameter("p_id"); <!--전 페이지에서 넘긴 상품의 id값을 가져온다.-->
	ProductDAO dao = new ProductDAO(); <!--sql문을 실행할 dao를 불러오고-->
	ProductMainVO bag1 = dao.one(p_id); <!--상품의 아이디 값을 이용해서 vo 가방에 담아놓는다.-->

	ProductSubVO bag2 = new ProductSubVO();
	ArrayList<ProductSubVO> list1 = dao.subcon(p_id);
	String[] sub = new String[list1.size()]; <!--옵션들에 대한 정보 전체를 리스트에 담아 놓는다.-->
	for (int i = 0; i < list1.size(); i++) { <!--가져온 값을 사용하기 위해서 배열을 만들어 값을 담아주었다.-->
		bag2 = list1.get(i);
	...
	//총합계산
				...
				if (user_id == "null") {
					...
					$('#go-db')
							.click(
									function godb() {
										con = con1 + '<br>' + con2 + '<br>' + con3 + '<br>' + con4 + '<br>';
										prc = end_price
										console.log(con)
										location.href = 'product_add.jsp?p_id=PC001&ps_content=' + con + '&ps_price=' + prc
									})
				}
				...
				$('#login').click(function() {
					if (user_id == "null") {
						...
						$.ajax({
							url: 'logout.jsp',
							success: function() {
								loction.href = "product_phone.jsp";
							}
						})
					}
				})
...
<body id="page-top">
	...
	<div id="main-body">
		<div id="col-m-1">
			<img alt="" src="<%=bag1.getP_pic()%>" width=470px height=576px>
			<br>
			<br>
			<br> <img alt="" src="img/HDbar.png" width=480px height=180px>
		</div>
		<div id="col-m-2">
			<div id="col1-1">
				<h1>iPhone 11 Pro 구입하기.</h1>
			</div>
			<div id="col1-2">
				<ul>
			...
			<div id="column2_1">
				...
				<div id="col2-choice">
					<div>
						<input type="radio" name="bt1" id="dp-choice1"
							value="<%=prc[0]%>" checked="checked"> <label
							id="change-val1"><%=sub[0]%></label>
					</div>
					<div>
						<input type="radio" name="bt1" id="dp-choice2"
							value="<%=prc[1]%>"> <label id="change-val2"><%=sub[1]%></label>
						<span>+<%=prc[1]%>원
						</span>
					</div>
				</div>
				<div id="col2-title">
					<h3>맘에 드는 마감을 선택하세요.</h3>
				</div>
				<div id="col2-choice">
					<div>
						<input type="radio" name="bt2" id="dp-choice3" checked="checked">
						<label id="change-val3"><%=sub[2]%></label>
					</div>
					<div>
						<input type="radio" name="bt2" id="dp-choice4"> <label
							id="change-val4"><%=sub[3]%></label>
					</div>
				</div>
				<div id="col2-title">
					<h3>원하는 용량을 선택하세요.</h3>
				...
				<div id="col2-choice">
					<div>
						<input type="radio" name="bt3" id="dp-choice5"
							value="<%=prc[4]%>" checked="checked"> <label
							id="change-val5"><%=sub[4]%></label>
					</div>
					<div>
						<input type="radio" name="bt3" id="dp-choice6"
							value="<%=prc[5]%>"> <label id="change-val6"><%=sub[5]%></label>
						<span>+<%=prc[5]%>원
						</span>
					</div>
				</div>
